...
class RedactionStrategy(MaskingStrategy):
    def mask(self, value: Any, **options) -> Any:
        if value is None:
            return None
        value_str = str(value)
        mask_char = str(options.get("mask_char", "*"))[:1] or "*"

        # Modo avanzado: conserva caracteres al inicio/final y usa una mascara fija.
        # Esto alinea la estrategia Python con las vistas SQL generadas por Enmask.
        has_pattern_options = any(key in options for key in ("keep_left", "keep_right", "mask_length"))
        if has_pattern_options:
            keep_left = max(int(options.get("keep_left", 0) or 0), 0)
            keep_right = max(int(options.get("keep_right", 0) or 0), 0)
            mask_length = max(int(options.get("mask_length", max(len(value_str) - keep_left - keep_right, 0)) or 0), 0)
            if keep_left + keep_right >= len(value_str):
                return value_str
            return value_str[:keep_left] + (mask_char * mask_length) + (value_str[-keep_right:] if keep_right else "")
