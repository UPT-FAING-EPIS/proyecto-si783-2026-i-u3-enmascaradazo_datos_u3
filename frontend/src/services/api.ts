import type {
  ...
  Summary, User, AuthResponse, DynamicQueryResponse, AuditLogEntry, MaskingPreviewResponse, SupportedDatabaseInfo, ConnectionTestResponse, ConnectionSchemaResponse, TablePreviewResponse, WorkbenchMaskPreviewRequest, WorkbenchMaskPreviewResponse
} from '../types';
...
export const getUsers = () => request<User[]>('/auth/users');

// ---- Connections ----
export const getSupportedDatabases = () => request<SupportedDatabaseInfo[]>('/connections/supported');
...
export const testConnection = (data: ConnectionCreate) =>
  request<ConnectionTestResponse>('/connections/test', { method: 'POST', body: JSON.stringify(data) });
export const testExistingConnection = (id: string) =>
  request<ConnectionTestResponse>(`/connections/${id}/test`, { method: 'POST' });
...
// ---- Masking Workbench ----
export const getWorkbenchSchema = (connectionId: string) =>
  request<ConnectionSchemaResponse>(`/workbench/connections/${connectionId}/schema`);
export const getWorkbenchRecords = (connectionId: string, target: string, limit = 20, graphElement?: string | null) => {
  const params = new URLSearchParams({ target, limit: String(limit) });
  if (graphElement) params.set('graph_element', graphElement);
  return request<TablePreviewResponse>(`/workbench/connections/${connectionId}/records?${params.toString()}`);
};
export const previewWorkbenchMask = (data: WorkbenchMaskPreviewRequest) =>
  request<WorkbenchMaskPreviewResponse>('/workbench/preview', { method: 'POST', body: JSON.stringify(data) });
...
export const previewJob = (id: string, limit = 20) =>
  request<MaskingPreviewResponse>(`/jobs/${id}/preview?limit=${limit}`);
...
export const deleteJob = (id: string, force = false) =>
  request<{ message: string }>(`/jobs/${id}?force=${force}`, { method: 'DELETE' });
