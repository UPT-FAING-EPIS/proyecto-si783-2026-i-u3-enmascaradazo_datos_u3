...
const tools = [
  ...
  },
  {
    ...
    description: "Lee el esquema de una conexión Enmask.",
    inputSchema: {
      type: "object",
      required: ["connection_id"],
      additionalProperties: false,
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        target_table: {
          type: "string",
          description: "Tabla, colección, clave/patrón, label o relación. Para SQL Server usar schema.tabla."
        },
        graph_element: {
          type: ["string", "null"],
        ...
        field_rules: {
          ...
          items: {
            ...
            properties: {
              ...
              strategy: {
                type: "string",
                enum: ["redaction", "hashing", "substitution", "nullification", "fpe", "perturbation"]
              },
              protection_mode: {
                type: "string",
                enum: ["virtual_view", "masked_view", "masked_column", "static_mask", "symmetric_encryption"],
...
async function callTool(name, args = {}) {
  switch (name) {
    ...
    case "enmask_test_connection":
      return enmaskRequest(`/api/v1/connections/${encodeURIComponent(args.connection_id)}/test`, {
        method: "POST"
      });
...
app.get("/mcp", (_req, res) => {
  res.status(405).json({
    error: "GET /mcp no implementa SSE en esta demo. Usa POST /mcp."
  });
});
