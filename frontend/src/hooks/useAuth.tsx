...
import { getCurrentUser, loginWithGoogle, loginWithPassword, registerWithPassword, setStoredToken, clearStoredToken } from '../services/api';
...
interface AuthContextValue {
  ...
  registerWithPassword: (email: string, password: string, name: string) => Promise<User>;
  logout: () => void;
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const registerWithPasswordFn = useCallback(async (email: string, password: string, name: string) => {
  ...
  const value = useMemo(
    ...
    [user, loading, loginWithGoogleFn, loginWithPasswordFn, registerWithPasswordFn, logout],
  );
