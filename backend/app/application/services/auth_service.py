...
class AuthService:
    ...
    def _resolve_role(self, email: str) -> str:
        admins = {e.strip().lower() for e in settings.admin_emails_list()}
    ...
    async def register_local(self, email: str, password: str, name: str) -> Dict[str, object]:
        ...
        user = User(
            email=normalized,
            name=display_name,
            role=self._resolve_role(normalized),
            picture=None,
    ...
    async def authenticate_local(self, email: str, password: str) -> Dict[str, object]:
        ...
        resolved_role = self._resolve_role(user.email)
    ...
    async def authenticate_google(self, token: str) -> Dict[str, object]:
        ...
        user = await self.get_user_by_email(normalized)
        if not user:
            user = User(email=normalized, name=name, picture=picture, role=role)
            user = await self._repository.create(user)
        else:
            changed = False
    ...
    def create_access_token(self, subject: str, email: str, role: str = "user", expires_in: int = 3600) -> str:
